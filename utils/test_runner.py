...
def run_all_tests():
    ...
    passed_tests = sum(1 for test in test_results.get('tests', []) if test.get('outcome') == 'passed')
    failed_tests = sum(1 for test in test_results.get('tests', []) if test.get('outcome') == 'failed')
    skipped_tests = sum(1 for test in test_results.get('tests', []) if test.get('outcome') == 'skipped')
...
def organize_tests_by_module(test_results: Dict[str, Any]) -> Dict[str, list]:
    ...
    for module_name in sorted(modules.keys()):
        ...
        sorted_modules[module_name] = sorted(modules[module_name], key=lambda x: x['name'])
